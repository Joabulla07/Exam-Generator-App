...
def validate_start_date_and_today(start_date) -> bool:
    ...
    if (
        start_date_convert.date() >= date_today.date()
        and validate_date_is_not_holiday_or_weekend(start_date)
    ):
        return True
    return False
...
def validate_date_end(end_date, start_date) -> bool:
    ...
    if (
        end_date_converted > start_date_converted
        and validate_date_is_not_holiday_or_weekend(end_date)
    ):
        return True
    return False
